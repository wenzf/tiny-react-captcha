...
exports.isStrangeDimension = exports.isLine = exports.onUserInputCB = exports.checkBehaviour = void 0;
const constants_1 = require("./constants");
const checkBehaviour = (behaviour) => {
    ...
    if (!isMouseOk && !isToucheOk && behaviour.current.keyboard.numbTab === 0)
        return false;
    if (behaviour.current.mouse.isNotTrusted !== 0 || behaviour.current.touch.isNotTrusted !== 0 || behaviour.current.keyboard.isNotTrusted !== 0)
        return false;
    if (isMouseOk || isToucheOk || isKeyboardOk)
        return true;
    return false;
};
exports.checkBehaviour = checkBehaviour;
const onUserInputCB = (type, e, behaviour, clientInfo) => {
    // @ts-expect-error asdf
    const clientScreen = e.view.screen;
    ...
    switch (type) {
        case constants_1.USER_INPUT_EVENTS.TOUCH_START:
            ...
            break;
        case constants_1.USER_INPUT_EVENTS.TOUCH_END:
            ...
            break;
        case constants_1.USER_INPUT_EVENTS.MOUSE_DOWN:
            ...
            break;
        case constants_1.USER_INPUT_EVENTS.MOUSE_UP:
            ...
            break;
        case constants_1.USER_INPUT_EVENTS.MOUSE_MOVE:
            if (behaviour.current.mouse.mouseMove !== undefined)
                behaviour.current.mouse.mouseMove += 1;
            if (e.isTrusted) {
                behaviour.current.mouse.isTrusted += 1;
            }
            else {
            ...
            break;
        case constants_1.USER_INPUT_EVENTS.KEY_DOWN:
            behaviour.current.keyboard.down += 1;
            // @ts-expect-error asdf
            if (e.key === 'Tab') {
                if (behaviour.current.keyboard.numbTab !== undefined)
                    behaviour.current.keyboard.numbTab += 1;
            }
            if (e.isTrusted) {
            ...
            break;
        case constants_1.USER_INPUT_EVENTS.KEY_UP:
            // @ts-expect-error asdf
            if (e.key === 'Tab') {
                if (behaviour.current.keyboard.numbTab !== undefined)
                    behaviour.current.keyboard.numbTab += 1;
...
exports.onUserInputCB = onUserInputCB;
function isLine(points) {
    ...
    return points.every(([x, y]) => {
        ...
        if (slope && yIntercept) {
            ...
            if (slopleByX !== undefined)
                return y === slopleByX + yIntercept;
        }
    });
}
exports.isLine = isLine;
const isStrangeDimension = (screenXorY) => {
    let initialValue = 0;
    for (let i = 0; i < screenXorY.length; i += 1) {
        if (initialValue === 0) {
            ...
            if (initialValue !== screenXorY[i])
                return true;
        }
    }
    return false;
};
exports.isStrangeDimension = isStrangeDimension;
