...
export function randomNumber(minInp, maxInp) {
    let min = minInp;
    let max = maxInp;
    if (minInp > maxInp) {
        min = maxInp;
...
export function randomString(length, charCollection) {
    let result = '';
...
export function getCharCollection(collectionStringType, captchaCharsCollection) {
    if (!collectionStringType)
        return abc123Friendly;
...
export const isDarkModePerefered = () => {
